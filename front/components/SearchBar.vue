<template>
  <div class="flex justify-center items-center min-h-screen">
    ...
    <div
      v-if="data && !loading"
      class="p-6 bg-gray-100 rounded-lg shadow-lg mt-6 w-full max-w-4xl flex-1 ml-96"
    >
      ...
      <div class="flex-1">
        <div
          v-if="data.word?.definitions?.length > 1"
          class="mt-4"
        >
          <h3 class="text-lg font-medium border-t pt-2">
            Other definitions
          </h3>
          <ul class="list-disc ml-4 space-y-1">
            <li
              v-for="(definition, index) in data.word.definitions.slice(1)"
              :key="index"
            >
              {{ definition.text }}
            </li>
        ...
        <div
          v-if="data.word?.etymology?.length > 1"
          class="mt-4"
        >
          <h3 class="text-lg font-medium border-t pt-2">
            Etymology
        ...
        <div
          v-if="data.word?.synonyms?.length > 1"
          class="mt-4"
        >
          <h3 class="text-lg font-medium border-t pt-2">
            Synonyms
          ...
          <ul class="list-disc ml-4 space-y-1">
            <li
              v-for="(synonym, index) in data.word?.synonyms"
              :key="index"
            >
              {{ synonym }}
            </li>
